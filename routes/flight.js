...
router.get("/displaybyid", function (req, res, next) {
  pool.query(
    "select F.*, (select S.statename from states S  where S.stateid=F.sourcestateid) as ss,(select S.statename from states S  where S.stateid=F.destinationstateid) as ds, (select C.cityname from city C where C.cityid=F.sourcecityid) as sc,(select C.cityname from city C where C.cityid=F.destinationcityid) as dc from  flights F where F.flightid=?",
    [req.query.fid],
    function (error, result) {
      if (error) {
        console.log(error);
...
router.get("/displayall", function (req, res, next) {
  ...
  if (!result) res.render("loginflight", { msg: "" });

  pool.query(
    "select F.*,(select C.cityname from city C where C.cityid=F.sourcecityid) as sc,(select C.cityname from city C where C.cityid=F.destinationcityid) as dc from  flights F",
    function (error, result) {
      if (error) {
...
router.get("/addnewflights", function (req, res, next) {
  var result = JSON.parse(localStorage.getItem("admin"));
  if (result) res.render("flightinterface", { msg: "" });
  else res.render("loginflight", { msg: "" });
});
...
router.get("/fetchallcity", function (req, res) {
  ...
  router.post("/addnewrecord", upload.single("logo"), function (req, res) {
    ...
    var fclass;
    if (Array.isArray(req.body.fclass)) fclass = req.body.fclass.join("#");
    else fclass = req.body.fclass;

    var days;
    if (Array.isArray(req.body.days)) days = req.body.days.join("#");
    else days = req.body.days;

    console.log(days);
    pool.query(
      "insert into flights(flightid,companyname,sourcestateid,sourcecityid,destinationstateid,destinationcityid,status,flightclass,sourcetiming,destinationtiming,days,logo)values(?,?,?,?,?,?,?,?,?,?,?,?)",
      [
        req.body.flightid,
      ...
      function (error, result) {
        if (error) {
          ...
          res.render("flightinterface", {
            msg: "Server Error,Record Not Submitted",
          });
        } else {
          res.render("flightinterface", {
...
router.post("/editdeleterecord", function (req, res) {
  ...
  if (req.body.btn == "Edit") {
    ...
    console.log(days);
    pool.query(
      "update flights set companyname=?,sourcestateid=?,sourcecityid=?,destinationstateid=?,destinationcityid=?,status=?,flightclass=?,sourcetiming=?,destinationtiming=?,days=? where flightid=?",
      [
        req.body.companyname,
        req.body.sourcestate,
        req.body.sourcecity,
...
router.post("/editpicture", upload.single("logo"), function (req, res) {
  pool.query(
    "update flights set logo=? where flightid=?",
    [req.file.originalname, req.body.flightid],
    function (error, result) {
      if (error) {
        res.redirect("/flight/displayall");
      } else {
        fs.unlinkSync("f:/flightenquiry/public/images/" + req.body.oldlogo);
        res.redirect("/flight/displayall");
      }
    }
  );
